...
,r_gestures.add(L,func("send").bind("{Browser_Back}"), "Back")          ;Right Stroke => Back
,r_gestures.add(R,func("send").bind("{Browser_Forward}"), "Forward")    ;Left Stroke  => Forward
,r_gestures.add(U,func("send").bind("#{Tab}"), "TaskView")              ;Up Stroke  => Task View

;===============================================================================================

m_gestures:=new gestures() ; These gestures will be activated by MButton
,m_gestures.add([R,U],func("msgbox").bind("RU"), "M-RU")    ;Chain Right->Up
,m_gestures.add([L,D,R],func("msgbox").bind("LDR"), "M-C")  ;Chain Left->Down->Right

,m_gestures.add([ ; Letter P = Up->Right->Down->Left, with Up being 1.5-10 times the size of other strokes
                {angle:U,size:[1.5,10]},{angle:R,size:1},{angle:D,size:1},{angle:L,size:1}   ]
               ,func("msgbox").bind("Letter P"), "M-P")
...
,m_gestures.add([   {angle:[UL,UR],size:[1.5,5]}, {angle:[R,DR],size:1}
              ...
              ,func("msgbox").bind("Letter R"), "M-R", {angle_tolerance:.125}       )
...
RButton up::send("{RButton" (r_gestures.end()!=1?" UP":"") "}") ;UP prevents key from getting stuck
; "gestures.end" returns 0 = No Gesture, 1 = Gesture Triggered, -1 = Unidentified Gesture

MButton::m_gestures.start()
MButton up::send("{MButton" (m_gestures.end()!=1?" UP":"") "}")